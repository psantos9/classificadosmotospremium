...
export const getAtualizaAnuncioSchema = () => z.object({
  ...
  ano: z.coerce.number({ required_error: REQUIRED_ERROR }).int('Ano inválido').gt(1900, 'Maior do que 1900').lte(new Date().getFullYear() + 1, `Menor ou igual a ${new Date().getFullYear() + 1}`),
