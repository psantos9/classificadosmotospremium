    ...
    <div class="card-section">
      <div class="max-w-xs w-full mx-auto flex flex-col items-center gap-4">
        ...
        <template v-if="phase === SignInPhase.Method">
          ...
          <button type="button" class="w-60 flex items-center justify-center gap-x-2 rounded-md bg-primary px-3.5 py-2.5 text-sm font-semibold shadow-sm" @click="phase = SignInPhase.Password">
          ...
          <button type="button" class="w-60 flex items-center justify-center gap-x-2 rounded-md bg-primary px-3.5 py-2.5 text-sm font-semibold shadow-sm" @click="phase = SignInPhase.Pin">
            <FontAwesomeIcon :icon="faTh" size="lg" />
            Entrar com um PIN
          </button>
        </template>
        <template v-if="phase === SignInPhase.Password">
          <span class="mt-4 title mx-auto">Insira a senha</span>
          <div class="max-w-[250px] w-full mx-auto relative flex items-center">
            <input id="password" type="password" name="password" class="block w-full rounded-md bg-white py-1.5 pl-10 pr-3 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-[var(--primary)] sm:pl-9 sm:text-sm/6">
            <FontAwesomeIcon :icon="faKey" size="sm" class="absolute pointer-events-none top-1/2 -translate-y-1/2 left-3 text-gray-400" />
          </div>
          <button type="button" class="mt-4 w-full md:w-40 flex items-center justify-center gap-x-2 rounded-md bg-primary px-3.5 py-2.5 text-sm font-semibold shadow-sm" @click="phase = SignInPhase.Method">
            Confirmar
